            ...
            @method("PUT")
                ...
                <div class="card-body">
                    ...
                    <input type="text" name="testName" id="testName" class="form-control my-3 @error("testName")
                        is-invalid
                    @enderror" value="{{ $test->name }}"  placeholder="Fill your task">
                    ...
                    @foreach ($projects as $project)
                        <option value="{{ $project->id }}"  @if ($test->project == $project->id)
                            selected
                    ...
                    <select name="priority" id="" class="form-control @error("priority")
                        is-invalid
                    @enderror">
                        <option value="true" @if ($test->priority == "true")
                            selected
                        @endif>Yes</option>
                        <option value="false" @if ($test->priority == "false")
                            selected
                        @endif>No</option>
                    </select>
                </div>
